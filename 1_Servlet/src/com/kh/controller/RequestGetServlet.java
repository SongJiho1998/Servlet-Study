...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 *  
		 * 첫번째 매개변수인 HttpServletRequest request에는 요청시 전달된 내용들이 담김(사용자가 입력한 값 , 요청
		 * 전송반식 , 요청한 사용자의 ip 등등 ..)
		 *   
		 * 두번째 매개변수인 HttpServletResponse 에는 요청 처리 후 응답을 할때 사용하는 객체.
		 * 
		 * 우선 , 요청을 처리하기 위해 요청시 전달된 값(사용자가 입력한 값) 들을 뽑는다.
		 * request의 parameter 영역안에 존재함 (key-value형태로 담겨있음)
		 * 
		 * 따라서 request의 parameter 영역으로부터 전달된 데이터를 뽑는 메소드
		 * - request.getParameter("키값") : String(그에 해당하는 value값) -> "무조건" 문자열로 반환됨(형변환필요)
		 * - request.getparameterValues("키값") : String[](그에해당하는 value값들)
		 * -> 하나의 key값으로 여러개의 value들을 받을경우(체크박스)문자열 배열형태로 반환됨.
		 */
		String name = request.getParameter("name"); // name = key값 // 비어있으면 ""(텍스트 상자가 빈경우 빈 문자열이 넘어감) // 송지호
		String gender = request.getParameter("gender"); // "M" , "F" (라디오 버튼이 체크되지 않은경우 null이 넘어옴)
		...
		// food -> 체크박스같은 복수개의 정보를 받을때는 배열로 받아야함.
		...
		// 보통의 흐름 : Service 의 메소드 호출시 뽑은 값들 전달 - Dao호출 - DB sql실행 - 결과반환 
